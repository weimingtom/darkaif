package programs
{
	...
	*/
	...
	public class Away3DXmlObject extends Sprite {
		...
		public function Away3DXmlObject() {
			//add scene
            addChild(view);
			//button
			...
			for (var fileno:int = 0; fileno < objectfilexml.file.length(); fileno++ ) {
				if (objectfilexml.file[fileno].idobject == filename) {
					...
					if (objectfilexml.file[fileno].type == '.md2') {
						...
						objfile = Md2.load(String(filename), { material:material, name:objectname } );
						//objfile.scale();
					...
					if (objectfilexml.file[fileno].type == '.ase') {
						...
						objfile = Ase.load(String(filename), { material:material, name:objectname } );
						//objfile.scale();
						view.scene.addChild(objfile);
					}
					...
					if (objectfilexml.file[fileno].type == '.dae') {
						...
						objfile = Collada.load(String(filename), { material:material, name:objectname } );
						view.scene.addChild(objfile);
					}
					
					if (objectfilexml.file[fileno].type == '.3ds') {
						trace('file type match. 3ds file');
						objfile = Max3DS.load(String(filename), { material:material, name:objectname } );
						view.scene.addChild(objfile);
					}
					
					if (objectfilexml.file[fileno].type == '.kmz') {
						trace('file type match. kmz file');
						objfile = Kmz.load(String(filename), { material:material, name:objectname } );
						view.scene.addChild(objfile);
					}
					
					if (objectfilexml.file[fileno].type == '.obj') {
						trace('file type match. kmz file');
						objfile = Obj.load(String(filename), { material:material, name:objectname } );
						view.scene.addChild(objfile);
					}
				}
			}
