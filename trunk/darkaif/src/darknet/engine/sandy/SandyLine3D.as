package
{
	...
	*  </listing>
	*/
	public class SandyLine3D extends Shape3D implements Primitive3D
	{
		/**
		...
		*/
		public function SandyLine3D ( p_sName:String=null, ...rest )
		{
			super ( p_sName );
		...
		*/
		...
		public function generatepoint3d(... arguments):void {
			var l_aPoints:Array = arguments[0];
			// --
			var i:int;
			var l:int = l_aPoints.length;
			...
			while( i < l )
			{
				//geometry.aVertex.push(  l_aPoints[int(i)].x, l_aPoints[int(i)].y, l_aPoints[int(i)].z );
				geometry.aVertex.push(new Vertex  (l_aPoints[int(i)].x, l_aPoints[int(i)].y, l_aPoints[int(i)].z) )
				//Vertex
				i++;
			}
			// -- initialisation
			...
			while( i < l-1 )
			{
				//geometry.setFaceVertexIds( i, i, i + 1);
				//geometry.aFacesVertexID.push()
				//geometry.setFaceVertexIds( geometry.getNextFaceID(), i, i + 1);
				//geometry.setFaceVertexIds( geometry.getNextFaceNormalID(), i, i + 1);
				//geometry.setFaceVertexIds( geometry.getNextFaceUVCoordID() , i, i + 1);
				//geometry.setFaceVertexIds( geometry.getNextUVCoordID() , i, i + 1);
				//geometry.setFaceVertexIds( geometry.getNextVertexID() , i, i + 1);
				//geometry.setFaceVertexIds( geometry.getNextVertexNormalID() , i, i + 1);
				i++;
			}
		}
