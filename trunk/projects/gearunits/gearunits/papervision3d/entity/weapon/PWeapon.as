package gearunits.papervision3d.entity.weapon 
{
	 ...
	 * @author Darknet
	 */
	public class PWeapon extends GlobalUnit
	{
		//{
		//GLOBAL VARS
		public static var projectiles:Vector.<PProjectile>; //global var class
		public static var units:Vector.<PStructureUnit>; //global var class
		public static var scene:Scene3D; //global var class
		//weapon fire from
		public var projectile:Vector.<PProjectile> = new Vector.<PProjectile>(); //global var class
			...
			if(BFIRE){
				...
				if (time > timemax) {
					...
					for (var p:int = 0; p < projectile.length; p++ ) {
						...
						if (bempty == false) {
							projectileclass = new PProjectile().projectile(projectile[p].CLASS);
						}
