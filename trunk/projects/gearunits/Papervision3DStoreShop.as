package  
{
	...
	import gearunits.models.PMeshStoreshopShirt;
	import gearunits.papervision3d.shop.PClothing;
	import gearunits.papervision3d.shop.PMeshBody;
	import gearunits.papervision3d.shop.PTexture;
	...
	public class Papervision3DStoreShop extends BasicView
	{
		...
		public var clothing:Vector.<PClothing> = new Vector.<PClothing>();
		...
		public var bodymesh:PMeshBody = new PMeshBody();
		...
		public function init_scene():void {
			...
			var bonetest:Papervision3DBoneVertexTest = new Papervision3DBoneVertexTest(wireMaterial);
			scene.addChild(bonetest);
			
			var building:PMeshStoreshop = new PMeshStoreshop(compatmat);
			
			var shirt:PMeshStoreshopShirt = new PMeshStoreshopShirt(compatmat);
			//scene.addChild(shirt)
		...
		public function loadmesh():void {
			...
			for (var c:int = 0; c < itemxml.item.clothing.length(); c++ ) {
				...
				var buildclothing:PClothing = new PClothing();
				...
				for (var m:int = 0; m < itemxml.item.clothing[c].mesh.material.texture.length(); m++) {
					var _tex:PTexture  = new PTexture();
		...
		public function dropboxlistselected(event:DropBoxEvent):void {
			...
			for (cc = 0; cc < clothing.length; cc++ ) {
				if ((clothing[cc].type == _type)&&(clothing[cc].weartype == _weartype)){
					try{
						scene.removeChild(clothing[cc].meshloader);
					}catch (e:Error) {
						//trace('error');
					}
			...
			scene.addChild(holderclothing.meshloader);
