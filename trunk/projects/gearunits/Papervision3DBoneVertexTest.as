package  {
	...
	public class Papervision3DBoneVertexTest extends TriangleMesh3D {
		...
		public function Papervision3DBoneVertexTest(material : MaterialObject3D ) {
			...
			for (var vertexno:int = 0; vertexno < geometry.vertices.length; vertexno++ ) {
				//this code needs to be independent point it will be hard to guess angle and reset position
				vertexframe.push(new Number3D(geometry.vertices[vertexno].x,geometry.vertices[vertexno].y,geometry.vertices[vertexno].z));
			}
			...
			if (bones['name']) {
				...
				*/
				...
				for (var vertexset:int = 0;vertexset < vertexf.length;vertexset++ ) {
					geometry.vertices[vertexset].x = vertexf[vertexset].x;
					geometry.vertices[vertexset].y = vertexf[vertexset].y;
					geometry.vertices[vertexset].z = vertexf[vertexset].z;
				}
