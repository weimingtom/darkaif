package programs
{
	...
	*/
	...
	public class Papervision3DXMLObject extends BasicView {
		...
		public function Papervision3DXMLObject() {
			...
			for (var fileno:int = 0; fileno < objectfilexml.file.length(); fileno++ ) {
				if (objectfilexml.file[fileno].idobject == filename) {
					...
					if (objectfilexml.file[fileno].type == '.md2') {
						...
						triangleMesh3d_md2.load(String(filename), material);
					...
					if (objectfilexml.file[fileno].type == '.ase') {
						...
						triangleMesh3d_ase = new Ase(null, (filename), 1);
					...
					if (objectfilexml.file[fileno].type == '.dae') {
						...
						triangleMesh3d_dae.load(filename);
					...
					if (objectfilexml.file[fileno].type == '.3ds') {
						...
						triangleMesh3d_3ds.load(filename);
					...
					if (objectfilexml.file[fileno].type == '.kmz') {
						...
						triangleMesh3d_kmz.load(filename);
